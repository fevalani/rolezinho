...
describe("LoginPage", () => {
  it("renders the login form", async () => {
    ...
    expect(await screen.findByText("Rolezinho Roots")).toBeInTheDocument();
  });
  ...
  it("shows password input", async () => {
    renderWithProviders(<LoginPage />);
    expect(
      await screen.findByPlaceholderText("Senha (mínimo 6 caracteres)"),
    ).toBeInTheDocument();
  });
  ...
  it("has toggle between login and signup", async () => {
    renderWithProviders(<LoginPage />);
    expect(
      await screen.findByText("Não tem conta? Criar agora"),
    ).toBeInTheDocument();
  });
});
